...
function M.new(options)
  ...
  local name = options.name or "widgets.disk"
  local position = options.position or "right"
  local disk_icon = icons.system_monitor and icons.system_monitor.disk or icons.cpu
  ...
  item:subscribe("routine", function()
    sbar.exec("/bin/df -k /", function(output)
      local fraction, percent = metrics.parse_disk_usage(output)
      if not fraction then return end
      item:push({ fraction })
      item:set({ graph = { color = usage_color(percent) }, label = "dsk " .. percent .. "%" })
    end)
  end)

  local controller = { key = "disk", names = { item.name } }
  ...
  function controller.open_activity_monitor() activity_monitor.open("disk") end
